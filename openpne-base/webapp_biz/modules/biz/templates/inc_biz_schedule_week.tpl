...
<a href="({t_url m=biz a=page_fh_biz_schedule_add})&amp;target_biz_group_id=({$target_id})">创建新计划</a>
<label for="title">预定</label>
...
<a href="({t_url m=biz a=page_g_home})&amp;w=({$w-1})&amp;target_c_commu_id=({$target_id})" title="上一周">＜</a>
<a href="({t_url m=biz a=page_g_home})&amp;target_c_commu_id=({$target_id})" title="本周">■</a>
<a href="({t_url m=biz a=page_g_home})&amp;w=({$w+1})&amp;target_c_commu_id=({$target_id})" title="下一周">＞</a>
({/t_form_block})
</div>
...
<option value="">现在时</option>
({section name=i loop=$hours})
<option value="({if $hours[i] < 10 })0({/if})({$hours[i]})">({$hours[i]})时</option>
({/section})
</select>
から
<select name="finish_time">
({section name=i loop=$mins})
<option value="({$mins[i]})">({$mins[i]})分后</option>
({/section})
</select>
まで
<input type="submit" class="input_submit" value="　添　加　" />
<a href="({t_url m=biz a=page_s_list})&amp;w=({$w-1})" title="前一周">＜</a>
<a href="({t_url m=biz a=page_s_list})" title="本周">■</a>
<a href="({t_url m=biz a=page_s_list})&amp;w=({$w+1})" title="下一周">＞</a>
({/t_form_block})
</div>
...
<input type="button" class="input_submit" onclick="location.href='({t_url m=biz a=page_s_edit_shisetsu})&amp;id=({$target_id})'" value="编　辑" />
<input type="button" class="input_submit" onclick="location.href='({t_url m=biz a=page_s_delete_shisetsu})&amp;target_id=({$target_id})&amp;sessid=({$PHPSESSID})'" value="删　除" />
({/if})
...
({if $cmd=='p' || $cmd=='h'})
<li><input type="button" class="input_submit" onclick="location.href='({t_url m=pc a=page_h_config_image})'" value="编辑头像" /></li>
({else})
<li><input type="button" class="input_submit" onclick="location.href='({t_url m=pc a=page_f_show_image})&amp;target_c_member_id=({$member_info.c_member_id})'" value="更多照片" /></li>
({/if})

</ul>
({/if})
<p class="text"><a href="({t_url m=pc a=page_f_home})&amp;target_c_member_id=({$member_info.c_member_id})">({$member_info.nickname})</a></p>
<p class="login_time">最终登录：({$member_info.last_login})</p>
({else})
...
<input type="submit" class="input_submit" value="更改昵称" /><br />
({/t_form_block})
</div>
<ul class="moreInfo button">
<li><input type="button" class="input_submit" onclick="location.href='({t_url m=pc a=page_h_config_image})'" value="更改头像" /></li>
<li><input type="button" class="input_submit" onclick="location.href='({t_url m=pc a=page_h_config_prof})'" value="个人资料变更" /></li>
</ul>
({/if})
...
<a href="({t_url m=biz a=page_fh_biz_schedule_calendar})({if $cmd == 'p' || $cmd == 'f'})&amp;target_id=({$member_info.c_member_id})({/if})">每月日历</a>

<input type="hidden" name="target_id" value="({$target_id})" />
<label for="title">预定计划</label> <input type="text" class="input_text" name="title" id="title" value="" size="30" />
<select name="start_date">
({foreach from=$daylist item=item})
<option value="({$item.year})-({$item.month})-({$item.day})"({if $item.now}) selected="selected"({/if})>({$item.month})/({$item.day})(({$item.dayofweek}))</option>
({/foreach})
...
<a href="({t_url m=pc a=page_`$cmd`_home})&amp;w=({$w-1})({if $target_id})&amp;target_c_member_id=({$target_id})({/if})" title="上一周">＜</a>
<a href="({t_url m=pc a=page_`$cmd`_home})({if $target_id})&amp;target_c_member_id=({$target_id})({/if})" title="今周">■</a>
<a href="({t_url m=pc a=page_`$cmd`_home})&amp;w=({$w+1})({if $target_id})&amp;target_c_member_id=({$target_id})({/if})" title="下一周">＞</a>
({/t_form_block})
</div>
({/if})
