...
function t_send_email($address, $subject, $body, $is_receive_mail=true, $from=ADMIN_EMAIL)
{
    ...
    if(is_ktai_mail_address($address)){
        $charset =  "iso-2022-jp";
        $mail_encode = "JIS";
        $base64_head = "ISO-2022-JP";
    } else{
        // yahoo.co.jp无法读取ＵＴＦ８邮件标题中的简体中文
        $mail_address = explode("@", $address);
        $domin = $mail_address[1]; 
        if(strtolower($domin) == 'yahoo.co.jp'){
            $charset =  "iso-2022-jp";
            $mail_encode = "JIS";
            $base64_head = "ISO-2022-JP";
        }else{
    ...
    $subject = mb_convert_encoding($subject, $mail_encode);
    $subject = str_replace(array($cr, $lf), "", $subject);
    $subject = '=?'.$base64_head.'?B?'.base64_encode($subject).'?=';

    // body (LF)
    if (MAIL_WRAP_WIDTH) {
        $body = t_wordwrap($body, MAIL_WRAP_WIDTH);
    }
    $body = mb_convert_encoding($body, $mail_encode);
