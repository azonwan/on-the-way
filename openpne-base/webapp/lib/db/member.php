...
function db_member_insert_session_id($c_menber,$c_session)
{
$date = array(
		'c_member_id' => intval($c_menber),
		'sessions_id' => $c_session,
	);
	return db_insert('c_invite_sessions', $date);
}

function db_member_get_session_id4c_member_id($c_menber)
{
  $sql = 'SELECT * FROM c_invite_sessions WHERE c_member_id = ?';
    $param = array(
        $c_menber,
    );
    return db_get_row($sql, $param);
}

function db_member_get_session4c_invite_sessions_id($c_invite_sessions_id)
{
  $sql = 'SELECT * FROM c_invite_sessions WHERE c_invite_sessions_id = ?';
    $param = array(
        $c_invite_sessions_id,
...
function db_member_get_c_invite_sessions_id4c_member_id($c_member_id, $sessions_id)
{
    $sql = 'SELECT c_invite_sessions_id FROM c_invite_sessions WHERE c_member_id = ? AND sessions_id =? ';
    $params = array($c_member_id, $sessions_id);
    return db_get_one($sql, $params);
}
function db_get_member_sex4c_member_id($c_member_id)
{
    $sql = 'SELECT c_profile_option_id FROM c_member_profile WHERE c_member_id = ? ';
    $params = array($c_member_id);
    $c_profile_option_id = db_get_one($sql, $params);
    $sql = 'SELECT value FROM c_profile_option WHERE c_profile_option_id = ? ';
    $params = array($c_profile_option_id);
    return db_get_one($sql, $params);
}
function db_get_username4c_member_id($c_member_id)
{
    $sql = 'SELECT username FROM c_username WHERE c_member_id = ? ';
    $params = array($c_member_id);
    return db_get_one($sql, $params);
