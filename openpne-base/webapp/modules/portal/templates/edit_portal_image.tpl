...
<dd class="insert">
<span class="textBtnS"><button type="button" onclick="insertImageTagToParentWindow('({$item.image_filename})')">插入</button></span>
<span class="textBtnS"><button type="button" onclick="deletePortalImage('({$item.image_filename})')">删除</button></span>
</dd>
</dl>
</div>
({/foreach})
...
<p class="listMove">
({$pager.total_num}) 中 ({$pager.start_num}) - ({$pager.end_num})显示
({if $pager.prev_page})<a href="?m=({$module_name})&amp;a=page_({$hash_tbl->hash('edit_portal_image')})&amp;page=({$pager.prev_page})&amp;target_area=({$target_area})">前面</a>({/if})
...
({/foreach})
({if $pager.next_page}) | <a href="?m=({$module_name})&amp;a=page_({$hash_tbl->hash('edit_portal_image')})&amp;page=({$pager.next_page})&amp;target_area=({$target_area})">后面</a>({/if})
</p>
...
<dl class="upload">
<dt class="upfile"><strong class="title">追加图片</strong></dt>
