...
class admin_do_update_c_cmd_caster extends OpenPNE_Action
{
    ...
    function execute($requests)
    {
        ...
        if (!($rss_url = OpenPNE_RSS::auto_discovery($url))) {
            admin_client_redirect('list_c_cmd_caster', 'CMD管理者URL无效');
        }

        $result = db_admin_update_c_cmd_caster($c_cmd_caster_id, $rss_url);
        if ($result) {
            admin_client_redirect('list_c_cmd_caster', 'CMD管理者URL已经变更');
        } else {
            admin_client_redirect('list_c_cmd_caster', 'CMD管理者URL无法变更');
        }
    }
}
