...
class admin_do_insert_c_banner extends OpenPNE_Action
{
    function execute($requests)
    {
        ...
        if (empty($upfile_obj) || $upfile_obj['error'] === UPLOAD_ERR_NO_FILE) {
            admin_client_redirect('insert_c_banner', '请选择图片');
        }

        if (!($image = t_check_image($upfile_obj))) {
            admin_client_redirect('insert_c_banner', '图片必须是' . IMAGE_MAX_FILESIZE . 'KB以内的GIF・JPEG・PNG文件');
        }
        ...
        admin_client_redirect('edit_c_banner', 'banner已经添加');
    }
}
