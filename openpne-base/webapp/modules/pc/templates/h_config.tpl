...
<th>手机邮件地址</th>
<td>
<a href="({t_url m=pc a=page_h_config_ktai})">登陆后也可使用手机版。登陆请按</a><br />
※手机信箱登陆后，也可以使用手机访问「({$smarty.const.SNS_NAME})」。
</td>
</tr>
({/if})
({if $smarty.const.OPENPNE_AUTH_MODE != 'slavepne'})
...
<td>
<ul>
<li><input type="radio" class="input_radio" name="is_receive_daily_news" id="is_receive_daily_news_1" value="1"({if $c_member.is_receive_daily_news == 1}) checked="checked"({/if}) /><label for="is_receive_daily_news_1">每日</label></li>
({if $daily_news_day_num})
<li><input type="radio" class="input_radio" name="is_receive_daily_news" id="is_receive_daily_news_2" value="2"({if $c_member.is_receive_daily_news == 2}) checked="checked"({/if}) /><label for="is_receive_daily_news_2">周({$daily_news_day_num})次 (({$daily_news_day_str}))</label></li>
({/if})
...
<td>
<ul>
<li><input type="radio" class="input_radio" name="schedule_start_day" id="schedule_start_day_0" value="0"({if $c_member.schedule_start_day == 0}) checked="checked"({/if}) /><label for="schedule_start_day_0">周日开始</label></li>
...
<li><input type="radio" class="input_radio" name="schedule_start_day" id="schedule_start_day_2" value="2"({if $c_member.schedule_start_day == 2}) checked="checked"({/if}) /><label for="schedule_start_day_2">本日开始</label></li>
</ul>
</td>
</tr>
({/if})
...
<td>
<ul>
<li><input type="radio" class="input_radio" name="IS_DISPLAY_SCHEDULE_HOME" id="is_display_schedule_home_0" value="1"({if $C_MEMBER_CONFIG.IS_DISPLAY_SCHEDULE_HOME}) checked="checked"({/if}) /><label for="is_display_schedule_home_0">显示</label></li>
<li><input type="radio" class="input_radio" name="IS_DISPLAY_SCHEDULE_HOME" id="is_display_schedule_home_1" value="0"({if !$C_MEMBER_CONFIG.IS_DISPLAY_SCHEDULE_HOME}) checked="checked"({/if}) /><label for="is_display_schedule_home_1">不显示</label></li>
</ul>
<p>※在我的个人页面上是否显示日程表。</p>
</td>
</tr>
...
<p>※是否在我的首页显示全部最新({$WORD_DIARY})。</p>
</td>
</tr>
<tr>
<th>({$WORD_COMMUNITY})最新帖子、活动</th>
<td>
<ul>
...
<p>※在我的首页显示全部最新({$WORD_COMMUNITY})信息。</p>
</td>
</tr>
({/if})
({if $smarty.const.USE_BOOKMARK_FEED})
<tr>
<th>收藏的最新({$WORD_DIARY})</th>
<td>
<ul>
...
<p>※在我的首页显示收藏夹中全部的最新({$WORD_DIARY})。</p>
</td>
</tr>
<tr>
<th>收藏的最新blog</th>
...
({* {{{ formTable *})
<div class="dparts formTable"><div class="parts">
<div class="partsHeading"><h3>({$WORD_DIARY})公开范围变更一览</h3></div>
...
<li><input type="radio" class="input_radio" name="public_flag_diary_all" id="public_flag_diary_all_friend" value="friend" /><label for="public_flag_diary_all_friend">对({$WORD_MY_FRIEND})公开</label></li>
