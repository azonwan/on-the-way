...
<img src="({t_img_url_skin filename=skin_login_open})" class="bg" alt="" />
...
<a href="({t_url m=pc a=page_o_public_invite})" id="button_new_regist"><img src="./skin/dummy.gif" alt="注册" /></a>
({/if})
...
<span class="password_query"><a href="({$smarty.const.SLAVEPNE_PASSWORD_QUERY_URL_PC})">&gt;忘记密码请访问这里</a></span>
({/if})
({else})
<span class="password_query"><a href="({t_url m=pc a=page_o_password_query})">&gt;忘记密码请访问这里</a></span>
({/if})
({if $SSL_SELECT_URL})
<br /><a href="({$SSL_SELECT_URL})">({if $HTTPS})标准(http)({else})SSL(https)({/if})在这里</a>
({/if})
</div>
({/t_form_block})
<div class="footer">
